...
```powershell
.\update_experiment.ps1 -TargetDirectory "output/reports/Study_6/Experiment_1"
```
...
After running one or more experiments, this script aggregates all their results and performs the final statistical analysis for the entire study. This is typically the final step.
...
*   **`experiment_manager.py`**: The primary controller for an entire experiment, functioning as a state machine. It continuously verifies the experiment's state and automatically takes the correct action (`NEW`, `REPAIR`, `REPROCESS`, or `MIGRATE`) until the experiment is `COMPLETE`. This self-healing design makes the pipeline resilient to interruptions.
    *   Usage: `python src/experiment_manager.py path/to/experiment`

*   **`orchestrate_replication.py`**: The engine for a **single** experimental run. It handles the sequential execution of the four pipeline stages (`build_llm_queries`, `run_llm_sessions`, `process_llm_responses`, and `analyze_llm_performance`) and generates the final `replication_report.txt`.

*   **`experiment_aggregator.py`**: The core script for hierarchical data aggregation. It recursively scans a study directory, performing a bottom-up summary and generating level-aware summary files (`REPLICATION_results.csv`, `EXPERIMENT_results.csv`, and `STUDY_results.csv`).
    *   Usage: `python src/experiment_aggregator.py path/to/study`

*   **`study_analysis.py`**: Performs the final, comprehensive statistical analysis (Two-Way ANOVA, post-hoc tests) on a study's master CSV file. It produces a detailed analysis log and publication-quality boxplots.
    *   Usage: `python src/study_analysis.py path/to/study`

*   **`run_bias_analysis.py`**: Calculates specific metrics for positional bias from a run's raw scores and injects them into the `replication_report.txt`'s JSON block for later analysis. Called automatically by `experiment_manager.py`.

*   **`replication_log_manager.py`**: Core utility for managing the human-readable `batch_run_log.csv`. Its `rebuild` command recreates the log from scratch, and `finalize` appends a summary after `experiment_aggregator.py` has run.

*   **`rebuild_reports.py`**: A powerful utility to regenerate all `replication_report.txt` files from the ground-truth `analysis_inputs` data. Useful for applying analysis fixes across an entire study.
    *   Usage: `python src/rebuild_reports.py path/to/study`

*   **`patch_old_experiment.py`**: A utility for historical data that ensures legacy runs have a proper `config.ini.archived` file. It is called automatically by `experiment_manager.py` during a data migration.

*   **`retry_llm_sessions.py`**: Finds and retries failed API calls in parallel, then automatically re-runs the downstream analysis pipeline to ensure reports are fully updated.

*   **`verify_experiment_completeness.py`**: A standalone diagnostic tool for auditing an experiment directory. Its logic is also used internally by `experiment_manager.py` for automated state verification.
---
