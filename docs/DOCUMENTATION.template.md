...
### Data Sourcing: Automated Fetching vs. Static Export

The pipeline can be initiated in two ways:

1.  **Automated Fetching (Recommended for New Research):** The `src/fetch_adb_data.py` script provides a fully automated method to scrape the latest data directly from the ADB website. It handles user authentication, navigates the site's internal API, and produces a clean, detailed raw data file (`data/sources/adb_raw_export_fetched.txt`). This is the preferred method for any new research.

2.  **Static Export (Required for Replication):** For perfect, bit-for-bit replication of this study's findings, researchers must use the static data export captured at the time of the original research: `data/sources/adb_raw_export.txt`. This file, along with a static validation report (`data/reports/adb_validation_report.csv`), ensures the entire pipeline remains fully deterministic.

The validation report was generated by a one-time run of `src/validate_adb_data.py`, which audits the raw data against live Wikipedia.

### The Automated Workflow

The automated process, driven by dedicated Python scripts and third-party software (Solar Fire), involves four main steps:
1.  **Filtering (`filter_adb_candidates.py`):** Takes a raw data export (either the static `adb_raw_export.txt` or a newly fetched file), the static validation report, and a pre-computed eminence score file as input. It filters candidates based on their validation status, birth year, and a valid birth time, while also removing duplicates. It then discards any candidates who lack an eminence score (logging their ARNs to a report file for auditability) and selects the top 5,000 subjects from the remaining pool based on their eminence rank.
2.  **Formatting (`prepare_sf_import.py`):** Converts the list of 5,000 subjects into the specific format required for import into the Solar Fire astrology software.
3.  **Integration (`create_subject_database.py`):** After data is processed by Solar Fire, this script integrates multiple data sources. It flattens the chart export, cross-references it with the original filtered list and eminence scores, and crucially, repairs character encoding issues originating from the export process. The output is a clean, UTF-8 encoded `subject_db.csv` file located in `data/processed/`.
4.  **Generation (`generate_database.py`):** This script assembles the final database. It loads its configuration (point weights, balance thresholds) and a library of neutralized text descriptions from data files. It then processes the subject data from the master `subject_db.csv`, calculates personality classifications, and constructs the final description text for each individual.

This combination of a one-time validation audit and a fully automated script chain ensures the final dataset is both high-quality and computationally reproducible.
...
{{grouped_figure:docs/diagrams/arch_prep_codebase.mmd | scale=2.5 | width=50% | caption=Data Preparation Code Architecture: The sequential execution flow of the three data processing scripts.}}
...
{{grouped_figure:docs/diagrams/logic_prep_generation.mmd | scale=2.5 | width=65% | caption=Logic for Database Generation (`generate_database.py`): The algorithm for loading configurations, calculating personality classifications, and assembling the final description text for each subject.}}
...
{{grouped_figure:docs/diagrams/flow_main_1_new_experiment.mmd | scale=2.5 | width=65% | caption=Workflow 1: Create a New Experiment, showing the main control loop and the internal replication pipeline.}}
...
{{grouped_figure:docs/diagrams/flow_main_4_migrate_experiment.mmd | scale=2.5 | width=100% | caption=Workflow 4: Migrate Old Experiment Data, a safe, non-destructive process for upgrading legacy data.}}
...
{{grouped_figure:docs/diagrams/flow_main_7_audit_study.mmd | scale=2.5 | width=100% | caption=Workflow 7: Audit a Study. Consolidated completeness report for all experiments in a study.}}
...
{{grouped_figure:docs/diagrams/flow_main_9_migrate_study.mmd | scale=2.5 | width=80% | caption=Workflow 9: Migrate a Study. A batch operation to upgrade all legacy experiments in a study.}}
...
{{grouped_figure:docs/diagrams/data_main_flow.mmd | scale=2.5 | width=70% | caption=Data Flow Diagram: Creation and transformation of data artifacts (files) by the main pipeline scripts.}}

#### Logic Flowcharts

These diagrams illustrate the scientific and procedural methodology at each level of the experimental hierarchy.

{{grouped_figure:docs/diagrams/logic_main_replication.mmd | scale=2.5 | width=65% | caption=Replication Logic: The scientific methodology for a single replication run.}}

{{grouped_figure:docs/diagrams/logic_main_experiment.mmd | scale=2.5 | width=65% | caption=Experiment Logic: The aggregation of multiple replication results to produce final experiment-level summaries.}}

{{grouped_figure:docs/diagrams/logic_main_study.mmd | scale=2.5 | width=70% | caption=Study Logic: The complete workflow for processing a study, from auditing and aggregation to final statistical analysis.}}
