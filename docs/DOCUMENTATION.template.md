...
The data generation process is a sequence of Python scripts interspersed with manual data processing using the third-party **Solar Fire** software. The pipeline is divided into pre-processing, manual processing, and post-processing stages.

**Pre-Processing (Automated)**
1.  **Filtering (`filter_adb_candidates.py`):** Takes a raw data export, the static validation report, and a pre-computed eminence score file as input. It filters candidates based on validation status, birth year, and a valid birth time, while also removing duplicates. It then discards any candidates who lack an eminence score and selects the top 5,000 subjects based on eminence rank.
2.  **Formatting (`prepare_sf_import.py`):** Converts the list of 5,000 subjects into the specific CQD format required for import into Solar Fire.

**Manual Processing (Solar Fire & LLM)**
...
3.  **Integration (`create_subject_db.py`):** This script integrates multiple data sources. It parses `sf_chart_export.csv`, cross-references it with the original filtered list and eminence scores, and repairs character encoding issues. The output is a clean, UTF-8 encoded master file: `data/processed/subject_db.csv`.
4.  **Generation (`generate_personalities_db.py`):** This final script assembles the experiment database. It loads configuration files (point weights, balance thresholds) and the neutralized description library. It then processes the master `subject_db.csv`, calculates personality classifications for each subject, and constructs the final `personalities_db.txt`.
