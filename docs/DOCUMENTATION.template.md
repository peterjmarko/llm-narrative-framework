...
The final `personalities_db.txt` used in the experiments is the product of a multi-stage data preparation pipeline designed for maximum reproducibility. The pipeline starts with two static foundational assets: a raw data export from the Astro-Databank research database (`data/sources/adb_raw_export.txt`) and a validation report (`data/reports/adb_validation_report.csv`).

The validation report was generated by a one-time run of the `src/validate_adb_data.py` script, which audits the raw data against live Wikipedia pages to confirm names and death dates. By using this static report as a filter, the entire subsequent pipeline remains fully deterministic.

The automated process, driven by dedicated Python scripts and the use of third-party software (Solar Fire), involves four main steps:
1.  **Filtering (`filter_adb_candidates.py`):** Takes the raw export, the static validation report, and a pre-computed eminence score file as input. It filters candidates based on their validation status, birth year, and a valid birth time, while also removing duplicates. It then discards any candidates who lack an eminence score (logging their ARNs to a report file for auditability) and selects the top 5,000 subjects from the remaining pool based on their eminence rank.
