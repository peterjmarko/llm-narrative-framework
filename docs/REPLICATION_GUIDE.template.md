...
- **🔬 Replication Guide** (this document) - Step-by-step procedures for reproducing or extending the study
...
{{grouped_figure:docs/diagrams/logic_matching_task.mmd | scale=2.5 | width=60% | caption=Figure S1: The Core Research Question: An LLM-driven matching task to detect a non-random signal.}}
...
*Table S1: Balance Category Threshold Ratios*
...
{{grouped_figure:docs/diagrams/flow_stimulus_generation.mmd | scale=2.5 | width=40% | caption=Figure S2: The four-stage process for generating the experimental stimuli.}}
...
{{grouped_figure:docs/diagrams/arch_project_overview.mmd | scale=2.0 | width=100% | caption=Figure S3: Project Architecture Overview. The framework consists of four main components: User-Facing Interface, Core Logic, Project Governance, and Data & Artifacts.}}
...
*Table S2: Comparison of Research Replication Paths*
...
{{grouped_figure:docs/diagrams/flow_prep_pipeline.mmd | scale=2.0 | width=37% | caption=Figure S4: Data Preparation Pipeline. The pipeline processes raw astrological data from ADB through multiple stages to create personalities_db.txt.}}
...
{{grouped_figure:docs/diagrams/flow_replication_paths.mmd | scale=2.5 | width=100% | caption=Figure S5: The Three Research Replication Paths.}}
...
{{grouped_figure:docs/diagrams/arch_prep_codebase.mmd | scale=2.5 | width=100% | caption=Figure S6: Data Preparation Code Architecture: The execution flow of the data processing scripts.}}
...
{{grouped_figure:docs/diagrams/flow_prep_pipeline.mmd | scale=2.5 | width=37% | caption=Figure S7: Data Preparation Workflow: The end-to-end pipeline from raw data extraction to the final generated databases, showing both manual and automated steps.}}
...
{{grouped_figure:docs/diagrams/flow_prep_1_qualification.mmd | scale=2.5 | width=74% | caption=Figure S8: Data Prep Flow, Stages 1 and 2 (Data Sourcing and Candidate Qualification).}}

{{grouped_figure:docs/diagrams/flow_prep_2_selection.mmd | scale=2.5 | width=80% | caption=Figure S9: Data Prep Flow, Stage 3 (LLM-based Candidate Selection).}}

{{grouped_figure:docs/diagrams/flow_prep_3_generation.mmd | scale=2.5 | width=100% | caption=Figure S10: Data Prep Flow, Stage 4 (Profile Generation).}}

<br>

Concurrent with these outputs, the individual scripts also generate summary reports as shown in the diagram below.

{{grouped_figure:docs/diagrams/flow_data_prep_reports.mmd | scale=2.5 | width=100% | caption=Figure S11: Data Prep Flow: Generated Reports.}}
...
{{grouped_figure:docs/diagrams/logic_prep_pipeline.mmd | scale=2.5 | width=48% | caption=Figure S12: Overall Logic for the Data Preparation Pipeline: A high-level view of the four main stages.}}

{{grouped_figure:docs/diagrams/logic_prep_find_links.mmd | scale=2.5 | width=70% | caption=Figure S13: Logic for Link Finding (`find_wikipedia_links.py`): The algorithm for finding Wikipedia URLs by scraping ADB and using a Wikipedia search fallback.}}

{{grouped_figure:docs/diagrams/logic_prep_qualify_subjects.mmd | scale=2.5 | width=80% | caption=Figure S14: Logic for Subject Qualification (`qualify_subjects.py`): The algorithm for validating Wikipedia/Wikidata entries, including redirect handling, disambiguation, and life status verification.}}

{{grouped_figure:docs/diagrams/logic_prep_eligible_candidates.mmd | scale=2.5 | width=65% | caption=Figure S15: Logic for Final Filtering (`select_eligible_candidates.py`): The algorithm for applying all deterministic data quality rules to create the final "eligible candidates" list.}}

{{grouped_figure:docs/diagrams/logic_prep_eminence_scoring.mmd | scale=2.5 | width=60% | caption=Figure S16: Logic for Eminence Scoring (`generate_eminence_scores.py`): The algorithm for batch processing, LLM interaction, and finalization of eminence scores.}}

{{grouped_figure:docs/diagrams/logic_prep_ocean_scoring.mmd | scale=2.5 | width=55% | caption=Figure S17: Logic for OCEAN Scoring (`generate_ocean_scores.py`): The algorithm for generating OCEAN personality scores for all eligible subjects. A robust pre-flight check ensures that interrupted runs can be safely resumed.}}

{{grouped_figure:docs/diagrams/logic_prep_final_candidates.mmd | scale=2.5 | width=29% | caption=Figure S18: Logic for Final Selection (`select_final_candidates.py`): The algorithm for finding the optimal cohort size by performing a slope analysis on a smoothed cumulative personality variance curve.}}

{{grouped_figure:docs/diagrams/logic_prep_neutralization.mmd | scale=2.5 | width=60% | caption=Figure S19: Logic for Delineation Neutralization (`neutralize_delineations.py`): The hybrid algorithm for rewriting texts. Fast mode bundles tasks for speed, while the robust default mode processes each item individually to guarantee completion.}}

{{grouped_figure:docs/diagrams/logic_prep_generation.mmd | scale=2.5 | width=70% | caption=Figure S20: Logic for Database Generation (`generate_personalities_db.py`): The algorithm for assembling the final description text for each subject.}}
...
{{grouped_figure:docs/images/sf_images/sf_setup_points.png | width=60% | caption=Figure S24: Solar Fire "Displayed Points" dialog configured with the 12 required chart points.}}
...
{{grouped_figure:docs/images/sf_images/sf_setup_import.png | width=80% | caption=Figure S25: Solar Fire "Edit ASCII Formats" dialog configured for the CQD Import format.}}
...
{{grouped_figure:docs/images/sf_images/sf_setup_export.png | width=80% | caption=Figure S26: Solar Fire "Export Chart Data" format dialog configured for the CQD Export format.}}
...
{{grouped_figure:docs/images/sf_images/sf_import_delete.png | width=95% | caption=Figure S27: Solar Fire "Chart Database" dialog with all charts selected for deletion.}}
...
{{grouped_figure:docs/images/sf_images/sf_import_charts.png | width=95% | caption=Figure S28: Solar Fire "Chart Import/Export" dialog configured to import the prepared data.}}
...
{{grouped_figure:docs/images/sf_images/sf_export_data.png | width=75% | caption=Figure S29: Solar Fire "Export Chart Data" dialog configured for the final chart data export.}}
...
{{grouped_figure:docs/diagrams/flow_experiment_study_workflow.mmd | scale=2.0 | width=35% | caption=Figure S21: Experiment & Study Workflow. The workflow uses personalities_db.txt to run experiments and compile study results.}}
...
{{grouped_figure:docs/images/effect_size_chart_example.png | scale=1.0 | width=85% | caption=Figure S22: Example of an Automated Effect Size Chart. This stratified chart visualizes the 'Goldilocks Effect,' showing how the effect size (η²) of the `mapping_strategy` factor peaks at the medium difficulty level (k=10).}}
...
*Table S6: Troubleshooting Quick Reference*
...
**Table S5: Complete Pilot Testing Results (42 Models Evaluated)**
...
#### Table S7: Complete FDR-Corrected Results
