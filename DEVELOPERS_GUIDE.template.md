# Developer's Guide to the LLM Narrative Framework
...
### Step 3: Optional Test Asset Setup (Large Seed Data)

To run the complete test suite, including the validation for the data-driven candidate selection logic, you need to provide a large seed dataset. These files are generated by the pipeline and are not stored in Git.

*   **Action:** Manually copy your generated `eminence_scores.csv`, `ocean_scores.csv`, and `adb_eligible_candidates.txt` files into the `tests/assets/large_seed/` directory.
*   **Details:** For the exact file paths and structure, see the detailed note in the **[🧪 Testing Guide (TESTING_GUIDE.md)](TESTING_GUIDE.md)** in the "Data Preparation Pipeline" integration testing section.
*   **Impact:** If you skip this step, the data pipeline integration test will still run but will skip the specific validation for the cutoff logic.
...
The `scripts/` directory contains helper utilities for development, maintenance, and building documentation, organized into the following subdirectories:

-   [`analysis/`](scripts/analysis/): Contains one-off scripts for data analysis and diagnostics.
-   [`build/`](scripts/build/): Contains scripts related to the project's build and release process.
-   [`lint/`](scripts/lint/): Contains custom linters to enforce project-specific code quality standards.
...
*   **Code**: Modify the Python files in `src/` or `tests/`. Pre-commit hooks will handle formatting and linting.
*   **Data**: Handle modifications to data files with care, as they can affect the final results. Generated files in `data/processed/` and `data/intermediate/` should not be edited manually.

#### How to Work with Documentation

The project's documentation is managed through three distinct methods. For a complete, up-to-date list of all documents and their types, please refer to the **[Project Scope Report](output/project_reports/project_scope_report.md)**.

**1. Simple Markdown Files**

-   **Description:** These are standard `.md` files that are edited directly.
-   **Examples:** `DEVELOPERS_GUIDE.md`, `PROJECT_ROADMAP.md`.
-   **Workflow:** Edit the file, save it, and commit your changes.
...
-   **Workflow:**
    ...
    1.  Make your changes to the `.template.md` source or a diagram file in `docs/diagrams/`.
    2.  Run the build script to apply your changes to the final documents:
        ```powershell
        pdm run build-docs
        ```
...
-   **Description:** These are reports generated by specific project commands.
-   **Examples:** `CHANGELOG.md` (from `pdm run release`), `project_scope_report.md` (from `pdm run scope-report`).
-   **Workflow:**

    1.  Run the appropriate generator command to update the report.
    2.  To create a `.docx` version of the updated report, you must then run the main documentation build script:
        ```powershell
        pdm run build-docs
        ```
...
This section covers common technical issues that may arise during development, testing, and dependency management.
