...
Key Features:
-   **Dynamic Template Discovery**: Automatically discovers all `*.template.md`
    files in the project (excluding Git/environment directories), eliminating
    the need for hardcoded document lists.

-   **JOSS Paper Generation**: Special handling for `paper.md` to automatically
    detect `paper.bib`, resolving citations via Pandoc's `citeproc` filter
    to generate a fully formatted `paper.docx` with bibliography.
...
def discover_templates(project_root):
    """
    Dynamically discovers all .template.md and .template.bib files in the project.
    Returns a list of tuples: (template_rel_path, final_rel_path, display_name)
    """
    templates = []
    # Walk excluding specific directories
    exclude_dirs = {'node_modules', 'output', '__pycache__', 'linter_backups', '_archive', '.venv', '.git', '.pdm-locks', 'htmlcov'}
    
    for root, dirs, files in os.walk(project_root):
        # Modify dirs in-place to skip traversals
        dirs[:] = [d for d in dirs if d not in exclude_dirs and not d.startswith('.')]
        
        for file in files:
            is_md = file.endswith('.template.md')
            is_bib = file.endswith('.template.bib')
            
            if is_md or is_bib:
                template_rel_path = os.path.relpath(os.path.join(root, file), project_root)
                
                if is_md:
                    final_rel_path = template_rel_path.replace('.template.md', '.md')
                else:
                    final_rel_path = template_rel_path.replace('.template.bib', '.bib')
                    
                # For display name, use the final filename
                display_name = os.path.basename(final_rel_path)
                
                # Normalize paths to use forward slashes for consistency (cross-platform)
                template_rel_path = template_rel_path.replace(os.sep, '/')
                final_rel_path = final_rel_path.replace(os.sep, '/')
                
                templates.append((template_rel_path, final_rel_path, display_name))
...
def generate_warning_header(target_filename, template_filename):
    ...
    is_bib = target_filename.endswith('.bib')
...
def convert_to_docx(pypandoc, output_docx_path, project_root, source_md_path, extra_pandoc_args=None):
    ...
    while True:
        try:
            ...
            if extra_pandoc_args:
                final_pandoc_extra_args.extend(extra_pandoc_args)
...
def main():
    ...
    if args.check:
        ...
        discovered_templates = discover_templates(project_root)
        
        # Build check list from discovered templates
        docs_to_check = [(t, f) for t, f, _ in discovered_templates]
        # Add non-template files that act as sources (e.g. for diagrams)
        docs_to_check.append(('docs/PROJECT_ROADMAP.md', 'docs/PROJECT_ROADMAP.md'))
    ...
    # Use dynamic discovery if not already done in check block
    if 'discovered_templates' not in locals():
        discovered_templates = discover_templates(project_root)

    # Define which files have placeholders that might contain diagrams
    files_with_diagrams = [t for t, _, _ in discovered_templates]
    files_with_diagrams.append('docs/PROJECT_ROADMAP.md')
    ...
    documents_to_build = discovered_templates
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for md_rel_path in md_files_to_convert:
                ...
                # Special handling for paper.md to include bibliography
                extra_args = []
                if output_filename == 'paper.docx':
                    # The source path is the original location in the project structure
                    bib_path = source_path.with_suffix('.bib')
                    if bib_path.exists():
                        # Use absolute path and ensure citeproc is enabled
                        extra_args.append(f'--bibliography={str(bib_path.resolve())}')
                        extra_args.append('--citeproc')

                if not convert_to_docx(pypandoc, output_path, project_root, source_md_path=path_to_convert, extra_pandoc_args=extra_args):
                    sys.exit(1)
