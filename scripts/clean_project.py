...
#
# Filename: scripts/clean_project.py
...
-   **Intelligent Pruning**: Automatically deletes all previous archives, keeping only the one
    just created. This prevents clutter over time.
...
# Global variable for easier testing
...
def main():
    ...
    safe_exclusions = {project_root / '.venv', project_root / '.git'}
    ...
    with tqdm(total=len(all_patterns), desc="Analyzing", ncols=80) as pbar:
        for category, pattern in all_patterns:
            pbar.set_postfix_str(f"Searching for '{pattern}'")
            for path in project_root.glob(pattern):
                # Ensure we don't clean inside the virtual environment
                if path.exists() and not any(part in safe_exclusions for part in path.parents):
    ...
    # 4. Execute archiving and pruning if requested
    if args.execute:
        ...
        # --- Step 1: Identify all existing archives BEFORE creating the new one ---
        archives_to_delete = sorted(list(archive_dir.glob("cleanup_archive_*.zip")))
        
        # --- Step 2: Archive all targeted items into a new ZIP file ---
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        archive_filename = f"cleanup_archive_{timestamp}.zip"
        archive_path = archive_dir / archive_filename
        print(f"\n--- Archiving {format_size(total_size)} of items to {archive_filename} ---")
        all_paths_to_clean = [item[0] for sublist in found_items.values() for item in sublist]
        with zipfile.ZipFile(archive_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            with tqdm(total=len(all_paths_to_clean), desc="Archiving", ncols=80) as pbar:
        ...
        # --- Step 3: Prune all the old archives identified in Step 1 ---
        ...
        if archives_to_delete:
            ...
            for old_archive in archives_to_delete:
                try:
                    ...
                    print(f"  {Colors.RED}ERROR:{Colors.ENDC} Could not delete {old_archive.name}: {e}")
        else:
...
# === End of scripts/clean_project.py ===
