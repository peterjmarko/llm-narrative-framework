...
*   **Documentation Templates**: This refers to the high-level project documentation, not docstrings.
    -   Modify the source `.template.md` files (e.g., `README.template.md`, `docs/DOCUMENTATION.template.md`) and their diagram sources in `docs/diagrams/`.
    -   Modify other static Markdown files like this guide (`CONTRIBUTING.md`) as needed.

    > **A Note on Generated Documents:** The final Markdown files (e.g., `README.md`, `TESTING.md`) are generated from their corresponding `.template.md` source files and are automatically set to **read-only** by the build script. If you try to edit one of these files directly, your editor will warn you. This is an intentional safety feature to prevent accidental edits. **Always make your changes to the `.template.md` file, and then run `pdm run build-docs` to apply them.**
