...
import src.study_analysis as run_anova_module

class TestStudyAnalysisScript(unittest.TestCase):
    """Tests for the main orchestration and helper functions of study_analysis.py."""
    ...
    def setUp(self):
        ...
        self.patch_get_config_list = patch('src.study_analysis.get_config_list')
        ...
        self.patch_get_config_section_as_dict = patch('src.study_analysis.get_config_section_as_dict')
        ...
        self.patch_app_config = patch('src.study_analysis.APP_CONFIG', new=MagicMock())
    ...
    def _run_main_and_capture_output(self, args):
        ...
        with patch('sys.stdout', new_callable=io.StringIO) as mock_stdout, \
             ...
             patch('src.study_analysis.plt'), \
             patch('src.study_analysis.sns'), \
             patch('src.study_analysis.nx'), \
             patch('src.study_analysis.sm'): # Patching sm here for consistency in main() tests
            ...
            with patch('logging.StreamHandler.__init__', autospec=True, side_effect=mock_stream_handler_init):
                with patch.object(sys, 'argv', ['study_analysis.py'] + args):
                    exit_code = 0
                    try:
    ...
    def test_create_diagnostic_plot_no_residuals(self):
        ...
        with patch('src.study_analysis.plt') as mock_plt, \
    ...
    def test_perform_analysis_single_factor_no_variation(self):
        ...
        with patch('src.study_analysis.create_and_save_plot') as mock_plot, \
    ...
    def test_perform_analysis_games_howell_fallback(self):
        ...
        with patch('src.study_analysis.sm') as mock_sm, \
             patch('src.study_analysis.pg') as mock_pg, \
             patch('statsmodels.stats.multicomp.pairwise_tukeyhsd', side_effect=ValueError("Tukey failed intentionally")), \
             patch('src.study_analysis.generate_performance_tiers') as mock_generate_tiers, \
             patch('logging.warning') as mock_log_warning, \
             patch('logging.info') as mock_log_info, \
             patch('src.study_analysis.create_diagnostic_plot'), \
             patch('src.study_analysis.create_and_save_plot'): # Patch plotting functions to prevent FileNotFoundError
    ...
    def test_perform_analysis_exception_handling(self):
        with patch('src.study_analysis.ols', side_effect=Exception("OLS error")), \
    ...
    def test_perform_analysis_tukey_success_and_tiers(self):
        ...
        with patch('src.study_analysis.sm') as mock_sm, \
             ...
             patch('src.study_analysis.generate_performance_tiers') as mock_tiers, \
             patch('src.study_analysis.create_diagnostic_plot'), \
             patch('src.study_analysis.create_and_save_plot'), \
             self.assertLogs('root', level='INFO') as cm:
    ...
    def test_perform_analysis_no_significant_factors(self):
        ...
        with patch('src.study_analysis.sm') as mock_sm, \
             patch('src.study_analysis.generate_performance_tiers') as mock_tiers, \
             patch('src.study_analysis.logging.info') as mock_log_info, \
             patch('src.study_analysis.create_diagnostic_plot'), \
             patch('src.study_analysis.create_and_save_plot'): # Patch plotting functions
    ...
    def test_generate_performance_tiers_no_cliques(self):
        with patch('src.study_analysis.nx') as mock_nx, self.assertLogs('root', level='INFO') as cm:
    ...
    def test_main_argparse_help(self):
        ...
        self.assertIn("python src/study_analysis.py path/to/your/study_folder/", help_output)
