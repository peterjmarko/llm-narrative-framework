...
# --- Validate and clean up large seed assets ---
...
# Clean up the now-generated `adb_eligible_candidates.txt` if it exists from an old run
$generatedLargeSeedFile = Join-Path $largeSeedBaseDir "intermediate/adb_eligible_candidates.txt"
if (Test-Path $generatedLargeSeedFile) {
    Remove-Item $generatedLargeSeedFile -Force
    Write-Host "  -> Removed surplus large_seed asset: adb_eligible_candidates.txt. This file is now generated by the test." -ForegroundColor Yellow
}

# Check for the required files and prompt the user if any are missing.
while ($true) {
    $missingFiles = @()
    foreach ($file in $requiredLargeSeedFiles) {
        if (-not (Test-Path (Join-Path $largeSeedBaseDir $file))) {
            $missingFiles += $file
        }
    }

    if ($missingFiles.Count -eq 0) {
        break # All files are present, continue with setup.
    }

    Write-Host "`nFATAL: One or more required large seed assets are missing for the algorithm validation tests." -ForegroundColor Red
    Write-Host "Please generate these files by running the full data preparation pipeline on a large dataset and place them in the correct locations." -ForegroundColor Yellow
    foreach ($file in $missingFiles) {
        Write-Host "  - Missing: tests/assets/large_seed/data/$file"
    }
    Read-Host "`nPress Enter after you have copied the missing files to resume setup..."
}
