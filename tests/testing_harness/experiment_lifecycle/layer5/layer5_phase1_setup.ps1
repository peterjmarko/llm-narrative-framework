...
try {
    ...
    if (Test-Path $Layer4StudyDir) {
        ...
        if ($Layer4Experiments.Count -eq 4) {
            Write-Host "Found complete 2x2 factorial study from Layer 4 test assets. Copying to study directory..." -ForegroundColor Green
            ...
            Write-Host "Warning: Found incomplete factorial study ($($Layer4Experiments.Count)/4 experiments). Using fallback data." -ForegroundColor Yellow
        }
    }
    ...
    if (-not $Layer4ExperimentsFound) {
        ...
        foreach ($exp in $mockExperiments) {
            ...
            # Generate 3 replications per experiment
