...
summary CSV generated by `compile_study_results.py` and performs a rigorous,
...
-   **Two-Way ANOVA**: For each performance metric, it conducts a Two-Way ANOVA
    to test for significant main effects from the experimental factors, and
    reports effect sizes (eta-squared).
...
The script generates a comprehensive `STUDY_analysis_log.txt`, publication-quality
boxplots, and diagnostic plots, all saved to an `anova/` subdirectory within
the specified study path.

Usage (typically called by process_study.ps1):
    python src/analyze_study_results.py /path/to/study_directory
"""
