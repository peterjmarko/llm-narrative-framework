...
#
# Filename: src/study_analyzer.py

"""
Comprehensive Statistical Analyzer for Study Results.

This script is the final stage of the analysis pipeline. It takes the master
summary CSV generated by `aggregate_experiments.py` and performs a rigorous,
automated statistical analysis to identify significant effects and performance
differences.

Key Statistical Features:
-   **Data Sanitization**: Applies canonical names and user-friendly display names
    from `config.ini` to ensure consistent grouping and clean, readable output.
-   **Reliability Filtering**: Excludes models from the analysis that fail to
    meet a configurable threshold for valid responses, ensuring data quality.
-   **Factorial ANOVA**: For each performance metric, it conducts a full factorial
    ANOVA to test for significant main effects and interaction effects, reporting
    effect sizes (eta-squared) for all terms.
-   **Assumption Checking**: Automatically generates Q-Q plots of residuals for
    each ANOVA model to allow for visual inspection of the normality assumption.
-   **Intelligent Post-Hoc Testing**: If a factor is significant, it first attempts
    a Tukey HSD test. If assumptions are not met, it automatically falls back
    to the more robust Games-Howell test.
-   **Advanced Performance Grouping**: Uses a graph-based clique-finding algorithm
    (`networkx`) on post-hoc results to identify statistically sound performance
    tiers, avoiding the pitfalls of simpler grouping methods.

Generated Artifacts:
The script generates a comprehensive `STUDY_analysis_log.txt` and diagnostic
Q-Q plots, all saved to an `anova/` subdirectory. Publication-quality boxplots
are saved to the `anova/` directory and also copied to the project's central
`docs/images/` directory for use in documentation.
