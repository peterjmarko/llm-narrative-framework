...
def create_and_save_plot(df, metric_key, display_metric_name, factor, p_value, output_dir, factor_display_map):
    ...
    sns.boxplot(ax=ax, y=plot_factor, x=metric_key, data=df, order=order, orient='h', palette="coolwarm", legend=False)
...
def perform_analysis(df, metric_key, all_possible_factors, output_dir, sanitized_to_display_map, metric_display_map, factor_display_map):
    ...
    if not active_factors:
        ...
        if df[all_possible_factors[0]].nunique() == 1:
                create_and_save_plot(df, metric_key, display_metric_name, all_possible_factors[0], float('nan'), output_dir, factor_display_map)
        return
...
def main():
    ...
    logging.info("generated by the run_anova.py script. The analysis is performed on the master `STUDY_results.csv`")
