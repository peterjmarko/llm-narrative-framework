...
"""
Calculates positional bias metrics and updates the replication's metric file.

This script is a post-processor that runs after the initial performance analysis
for a replication is complete. It reads the raw `all_scores.txt` and
`all_mappings.txt` files to calculate positional bias metrics.

It then reads the `replication_metrics.json` file (generated by the previous
stage), injects its calculated metrics, and overwrites the file. This augmented
JSON file becomes the final, authoritative source of all metrics for the
replication run.
