...
"""
Re-process and Re-analyze Experiment Runs

Purpose:
This script orchestrates the re-processing of one or more completed experimental
runs. It is useful for applying updated analysis logic to existing raw data
without re-running costly LLM queries.

It can operate in two modes:
1.  **Parent Directory Mode**: Scans a directory (e.g., 'output') and re-analyzes
    every `run_*` subfolder found within it.
2.  **Single Run Mode**: Re-analyzes only one specific `run_*` directory.

Workflow:
- For each targeted 'run_*' directory:
    ...
    c. It captures the new analysis and regenerates the `replication_report_*.txt`
       file, preserving necessary header information from the original report.
- If run in Parent Directory Mode, it calls `compile_results.py` at the end to
  update the master `final_summary_results.csv`.

Command-Line Usage:
    # Re-process all runs located in the default 'output' directory
    python src/reprocess_runs.py --parent_dir output

    # Re-process only a single specific run directory
    python src/reprocess_runs.py --run_dir path/to/specific/run_dir
"""

# === Start of src/reprocess_runs.py ===
