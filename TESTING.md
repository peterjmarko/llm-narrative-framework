...
{{diagram:docs/diagrams/test_philosophy_overview.mmd | scale=2.5 | width=80% | caption=The Two-Part Testing Philosophy: Separating the validation of the core scientific methodology from the software's execution flow.}}
...
{{diagram:docs/diagrams/test_sandbox_architecture.mmd | scale=2.5 | width=80% | caption=The Integration Test Sandbox Architecture: All integration tests run in a temporary, isolated environment, ensuring the main project data is never modified.}}
...
`Profile Generation Algorithm`      `N/A`           COMPLETE. Standalone `pytest` suite provides bit-for-bit
